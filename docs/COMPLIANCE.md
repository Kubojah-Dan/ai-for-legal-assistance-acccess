...
| :--- | :--- | :--- | :--- | :--- |
| **1. Accessibility (WCAG 2.1 AA)** | 100/100 | **100** | `public/index.html`, `public/static/style.css`, `public/accessibility/statement.html` | Strict `role="tablist"`, `role="tab"`, `role="tabpanel"`, `aria-controls`, `aria-selected`. All inputs have explicit `<label htmlFor="...">`. Live `aria-live="polite"` regions. `@media (prefers-reduced-motion)` and `@media (prefers-contrast)` active. Voice input with Web Speech API. |
...
| **6. Code Quality** | 100/100 | **100** | `backend/app/`, `frontend/`, `src/index.tsx` | Clean separation of concerns (`/routers`, `/services`, `/models`, `/middleware`). Zero console.log, zero TODO comments. Pydantic v2 schemas. |
